...
def normalize(s: str) -> str:
    if s is None:
        return ""
    s = str(s).strip()
    # Unificar separadores y espacios raros
    s = re.sub(r"\s+", " ", s)
    return s
...
def clean_for_filename(s: str) -> str:
    s = normalize(s)
    # Sacar caracteres inválidos para Windows
    s = re.sub(r'[<>:"/\\|?*\n\r\t]', " ", s)
...
def read_pdf_text(pdf_bytes: bytes) -> str:
    ...
    # Fallback si el texto es casi nulo
...
# -----------------------------
# BL detection (clave para Exolgan)
# -----------------------------
BL_PATTERNS = [
    # Etiquetados
    ...
    # BL "ocultos" / forwarder refs típicas:
    # Ej: GSZBUE25112140 (3-5 letras + 3 letras + 8-10 dígitos)
    r"\b([A-Z]{3,5}[A-Z]{3}\d{6,10})\b",

    # Contenedores (solo para debug; NO lo usamos como BL)
    # r"\b([A-Z]{4}\d{7})\b",
]

# Para evitar confundir contenedores con BL (CAUU7870064, FFAU3799866, etc.)
...
def detect_bl(text: str):
    ...
    - bl_detectado: string o None
    - debug_tokens: lista de tokens candidatos encontrados
    """
    if not text:
        return None, []
    ...
    # Capturar candidatos con patrones
    ...
    # Filtrar contenedores del set de candidatos "genéricos"
    filtered = []
    for cand in debug_tokens:
        # Si es claramente un contenedor, descartalo como BL principal
    ...
    # - Priorizar los que tengan aspecto de BL forwarder (GSZBUE...)
    # - Evitar los demasiado cortos o con muchas barras raras
    if not debug_tokens:
        return None, []

    # Ranking simple por longitud y por patrón GSZBUE-like
    def score(x):
        s = 0
...
def detect_proveedor(text: str) -> str:
    ...
    # Fallback: intentar “razón social” por primera línea con letras
    lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
    for ln in lines[:25]:
        up = ln.upper()
        # Evitar líneas que son solo números/códigos
        if len(up) < 5:
            continue
...
def build_mapping(df: pd.DataFrame):
    ...
    # Identificar columna de operación
    op_col = None
...
def process_pdfs(pdf_files, mapping, output_dir: Path):
    ...
    for f in pdf_files:
        ...
        if status == "OK":
            ...
            # Para depurar, dejamos proveedor siempre
            if bl_detectado:
...
st.title("Renombrar Facturas PDF por HBL/MBL/BL → Operación + Proveedor")

st.write(
    "Subí el **Excel** con la columna de **operación** y alguna columna con **BL/MBL/HBL**, "
    "y luego subí los **PDFs**. El sistema detecta el BL (incluye formato Exolgan tipo `GSZBUE...`), "
    "cruza contra el Excel y renombra."
)
...
with st.expander("Notas (si un PDF es escaneado)"):
    st.write(
        ...
        "- En ese caso necesitás OCR (Tesseract). Si querés, te lo integro también."
    )